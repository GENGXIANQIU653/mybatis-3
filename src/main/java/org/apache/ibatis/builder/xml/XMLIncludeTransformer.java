...
public class XMLIncludeTransformer {
  ...
  private void applyIncludes(Node source, final Properties variablesContext, boolean included) {
    ...
    /**
     * <1> 如果是 <include /> 标签，source.getNodeName 一般是select
     */
    if ("include".equals(source.getNodeName())) {
      // <1.1> 获得 <sql /> 对应的节点，见detail
      Node toInclude = findSqlFragment(getStringAttribute(source, "refid"), variablesContext);

      // <1.2> 获得包含 <include /> 标签内的属性，见detail
      Properties toIncludeContext = getVariablesContext(source, variablesContext);

      /**
       * <1.3> 递归调用 #applyIncludes(...) 方法，继续替换。注意，此处是 <sql /> 对应的节点，并且 included 参数为 true
       * 在 <sql /> 节点，会进入 <2> 这块逻辑
       *
       * <2.1> 处，因为 included 为 true ，所以能满足这块逻辑，会进行执行。如 <sql id="123" lang="${cpu}"> 的情况，lang 属性是可以被替换的。
       * <2.2> 处，遍历子节点，递归调用 #applyIncludes(...) 方法，继续替换
